...
export class FieldComponent extends ComponentBase {
      ...
      this.store.pipe(
        select(fromAuths.getUserId)).subscribe(
          id => {
            ...
            this.userDefinedColumnSetting =
              new UserDefinedColumnSetting(`${id}_UserDefinedColumnsField`, 'name,description,type,defaultValue');
            this.getFields();
  ...
  getFields() {
    this.store.dispatch(new fieldActions.GetFields(new PagingFilterCriteria(this.pageNumber + 1, this.pageSize)));
    this.handleSubscription(
  ...
  changeSelectedPage(page: number) {
    this.store.dispatch(new fieldActions.GetFields(new PagingFilterCriteria(page + 1, this.pageSize)));
  }
