...
{
    public class GetExtendedCutomersQuery : IGetExtendedCutomersQuery
    {
        private readonly HarveyCRMLoyaltyDbContext _dbContext;
        public GetExtendedCutomersQuery(HarveyCRMLoyaltyDbContext dbContext)
        {
            _dbContext = dbContext;
        }
        public GetExtendedCutomersResponse Execute(GetExtendedCutomersRequest request)
        {
            ...
            var response = new GetExtendedCutomersResponse();
            ...
            if (query != null && query.Any())
            {
                var queryCustomer = query
                    ...
                    .Select(x => new ExtendedCustomersModel
                    {
                        ...
                        ExtendedDate = x.Customer != null ? x.Customer.CreatedDate : null,
                        CustomerCode = x.Customer != null ? x.Customer.CustomerCode : ""
                    })
                .OrderByDescending(x => x.JoinedDate)
                .AsQueryable();
                var result = PagingExtensions.GetPaged<ExtendedCustomersModel>(queryCustomer, request.PageNumber, request.PageSize);
