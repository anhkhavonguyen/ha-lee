...
export class CategoryComponent extends ComponentBase {
    ...
    this.handleSubscription(this.store.pipe(
      ...
      .subscribe(
        (id: string) => {
          ...
          this.userDefinedColumnSetting = new UserDefinedColumnSetting(`${id}_UserDefinedColumnsCategory`, 'name,description');
          this.getCategories();
  ...
  getCategories() {
    this.store.dispatch(new categoryActions.GetCategories(new PagingFilterCriteria(1, this.pageSize)));
  ...
  changeSelectedPage(page: number) {
    this.store.dispatch(new categoryActions.GetCategories(new PagingFilterCriteria(page + 1, this.pageSize)));
  }
