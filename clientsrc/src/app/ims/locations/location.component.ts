...
export class LocationComponent extends ComponentBase {
    ...
    this.handleSubscription(this.store.pipe(
      ...
      .subscribe(
        (id: string) => {
          ...
          this.userDefinedColumnSetting = new UserDefinedColumnSetting(`${id}_UserDefinedColumnsLocation`, 'name,address,locationType');
          this.getLocations();
  ...
  getLocations() {
    this.store.dispatch(new locationActions.GetLocations(new PagingFilterCriteria(1, this.pageSize)));
  ...
  changeSelectedPage(page: number) {
    this.store.dispatch(new locationActions.GetLocations(new PagingFilterCriteria(page + 1, this.pageSize)));
  }
