...
export class FieldTemplateEffects {
    ...
    @Effect()
    getSelectedColumns$: Observable<Action> = this.action$.pipe(
        ofType(fieldTemplateActions.FieldTemplateActionTypes.GetSelectedColumns),
        map((action: fieldTemplateActions.GetAllSelectedColumns) => action.payload),
        mergeMap((key: string) => this.appSettingService.getBy(key).pipe(
            map((result: AppSettingModel) => (new fieldTemplateActions.GetSelectedColumnsSuccess(result)))
        ))
    );
        ...
        mergeMap((fieldTemplate: FieldTemplateModel) =>
            this.fieldTemplateService.update(fieldTemplate).pipe(
                map(updatedFieldTemplate => {
                    ...
                    return (new fieldTemplateActions.UpdateFieldTemplateSuccess(fieldTemplate));
                }),
                catchError(error =>
